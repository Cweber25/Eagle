...
image[10] = '20_library-exterior2.jpg';
...
const paragraph = {
  ...
  5: "Schar is a classroom oriented building on the Ashland campus. Many \
      classes relating to history, law, education, and psychology tend to \
      take place in this building. Due to the size of the building, many \
      classes from other fields will end up taking place here, so this tends \
      to be more of a general building. Another area in the College of \
...
function getDailyImage() {
  document.getElementById("img").src = `images/${image[answerInd]}`
}

function showPopup() {
  // set popup to visible and tint background
  document.getElementsByClassName('popup')[0].style.visibility = 'visible';
  document.getElementById("tinter").style.visibility = 'visible';
  
  // insert proper image and text
  document.getElementById("popImg").src = `images/${image[answerInd]}`;
  document.getElementById("pg").innerHTML = paragraph[answerInd]
}

function changeImage() {
    var answer = document.getElementById("answer").value;
    // if(answer == "Rec") {
    if(answer == awr[answerInd]) {
        showPopup()
